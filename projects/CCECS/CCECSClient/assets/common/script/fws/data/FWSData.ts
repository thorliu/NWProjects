 ...
 * @Last Modified by: 刘强
 * @Last Modified time: 2018-08-21 20:41:57
 */

module FWSData
{
	// export const USE_EVAL:boolean = false;
	...
	export function setValueFromPath(source: any, sourcePath: string, target: any, targetPath: string): void
	{
		...
		try
		{
			// if(FWSData.USE_EVAL)
			// {
			// 	eval("target" + targetPath + " = source" + sourcePath + ";");
			// }
			// else
			{
	...
	class DataBindManager
	{
		...
		private onPropertiesChange(e: DataPropertyChangeEventArgs, lk: DataBindLink, twoway: boolean): void
		{
			...
			else
			{
				...
				// if(FWSData.USE_EVAL)
				// {
				// 	if (e.propertyName.substr(0, 1) !== "["
				// 		&& e.propertyName.substr(0, 1) !== ".")
				// 	{
				// 		srcName = "." + srcName;
				// 	}
				// }
			...
			setValueFromPath(src, srcName, tag, tagName);
	...
	export function copyProperties(source: any, target: any, options?: any): void
	{
		...
		for (var k in options)
		{
			var v = options[k];
			// target[k] = source[v];
			FWSData.setValueFromPath(source, v, target, k);
		}
	}
...
// window["FWSData"] = FWSData;

// var testSource: any = {};
// var testTarget: any = {};
// testSource.info = {};
// testTarget.ctrl = {};
// testSource.info.name = "Liuqiang";

// debugger
// FWSData.setValueFromPath(testSource, ".info.name", testTarget, ".ctrl.nickName"); 