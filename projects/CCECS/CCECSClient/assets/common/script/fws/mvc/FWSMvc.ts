 ...
 * @Last Modified by: 刘强
 * @Last Modified time: 2018-09-06 18:23:02
 */
...
module FWSMvc
{
	/** 是否跟踪显示每个消息有哪些模块接收了 (正式版本应该关闭) */
	export const MVC_MSG_HANDLER_TRACE: boolean = false;

	/** 是否跟踪连接状态变化 (正式版本应该关闭) */
	export const MVC_CONNECTION_TRACE: boolean = false;

	/** 是否对context切换过程容错处理 (正式版本应该打开， 开发版本可以关闭) */
	export const MVC_CONTEXT_TRY_CATCH: boolean = true;

	/** 是否跟踪context切换的各模块性能 */
	export const MVC_CONTEXT_TRACE_TIME: boolean = true;

	//-------------------------------------- 状态系统

	export interface ICheckTraceMvcMsg
	{
		(msg: FWSMvc.FMessage<any>): boolean;
	}

	export var mvcMsgTraceChecker: ICheckTraceMvcMsg;
		...
		public onEnterContext(): void
		{
			for (var i: number = 0; i < this._mods.length; i++)
			{
				...
				if (MVC_CONTEXT_TRY_CATCH)
				{
					try
					{
						var modClassName: string = "(Timer) onEnterContext: " + FWSTool.Obj.getClassName(mod);
						if (MVC_CONTEXT_TRACE_TIME) X.time(modClassName);
						mod.onEnterContextMember();
						if (MVC_CONTEXT_TRACE_TIME) X.timeEnd(modClassName);
					}
					catch (err)
					{
						X.error(err);
		...
		public onLeaveContext(): void
		{
			for (var i: number = 0; i < this._mods.length; i++)
			{
				...
				if (MVC_CONTEXT_TRY_CATCH)
				{
					try
					{
						var modClassName: string = "(Timer) onLeaveContext: " + FWSTool.Obj.getClassName(mod);
						if (MVC_CONTEXT_TRACE_TIME) X.time(modClassName);
				...
				node !== this._current)
			{
				...
				if (this._current)
				{
					...
					for (var i: number = closeListCount - 1; i >= 0; i--)
					{
						...
						if (FWSEnv.DEBUG_CONTEXT_TRACE) X.color("blue", "(Context) onLeaveContext", closeContext.path);
						if (MVC_CONTEXT_TRY_CATCH)
						{
							try
							{
				...
				for (var i: number = 0; i < openList.length; i++)
				{
					...
					{
						if (FWSEnv.DEBUG_CONTEXT_TRACE) X.color("blue", "(Context) onEnterContext", openContext.path);
						if (MVC_CONTEXT_TRY_CATCH)
						{
							try
							{
				...
				if (sh) 
				{
					...
					X.color("blue", "Context goto", node.path);
				}
				else
				{
					X.color("blue", "Context goto(no history)", node.path);
				}
	...
	class FMessageRouter
	{
		...
		public connect(connection: IFMessageConnection): void
		{
			...
			else
			{
				...
				if (MVC_CONNECTION_TRACE) X.log("(路由) 接入", connection);
			}
		}
		...
		public disconnect(connection: IFMessageConnection): void
		{
			if (this._connections.indexOf(connection) >= 0)
			{
				...
				if (MVC_CONNECTION_TRACE) X.log("(路由) 断开", connection);
			}
		}
		...
		private push(msg: FMessage<any>): void
		{
			...
			{
				if (!(FWSMvc.mvcMsgTraceChecker && FWSMvc.mvcMsgTraceChecker(msg)))
				{
...
//NOTE: 下面这个函数决定消息对象是否向日志输出 (true=跳过)
FWSMvc.mvcMsgTraceChecker = (msg: FWSMvc.FMessage<any>): boolean =>
{

	if (",LoadingMsgsProgress,onWSCmdBeep,".indexOf(msg.key) >= 0) return true;
	if (msg.key === "WebSocketSend" && msg.data.id === 10101) return true;
	if (msg.key === "WebSocketOnReceive" && msg.data.id === 10101) return true;
