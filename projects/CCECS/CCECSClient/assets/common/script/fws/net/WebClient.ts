 ...
 * @Last Modified by: 刘强
 * @Last Modified time: 2018-08-23 10:43:49
 */
...
import WXWebClient = require('./WXWebClient');
...
{
	/** GET超时时间 */
	export const GET_OVERTIME: number = 1000 * 15;

	/** POST超时时间 */
	export const POST_OVERTIME: number = 1000 * 30;
	...
	{
		if(WXWebClient.get(uri, data, callbackTarget, successHandler, failHandler)) return;
		...
		{
			// console.log("request.ontimeout", request.status);
			if (request.readyState !== 4)
			{
				if (failHandler) failHandler.call(callbackTarget, request);
			}
	...
	export function post(
		...
		failHandler: Function): void
	{
		if(WXWebClient.post(uri, postData, callbackTarget, successHandler, failHandler)) return;
